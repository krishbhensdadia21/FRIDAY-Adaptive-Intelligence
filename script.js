...
// Initialize welcome message with voice greeting
...
clearBtn.onclick = async () => {
  ...
  const newRandomGreeting = GREETING_PHRASES[Math.floor(Math.random() * GREETING_PHRASES.length)];
  const welcomeDiv = document.createElement('div');
  welcomeDiv.id = 'welcomeArea';
  welcomeDiv.className = 'welcome-content';
  welcomeDiv.innerHTML = `
    <h2>${newRandomGreeting}</h2>
  `;
  chat.appendChild(welcomeDiv);
  welcomeArea = document.getElementById('welcomeArea');
  
  // Speak the new greeting
  speakGreeting(newRandomGreeting);
